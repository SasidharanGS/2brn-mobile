...
const ITEMS: MenuItem[] = [
  ...
  { href: '/more/timeline', icon: 'time-outline', label: 'Timeline', hint: 'What you did, capture by capture' },
  { href: '/more/saved', icon: 'bookmark-outline', label: 'Saved', hint: 'Items you shared into 2brn' },
  { href: '/more/instructions', icon: 'options-outline', label: 'Instructions', hint: 'Guide how entries are written' },
  { href: '/more/settings', icon: 'settings-outline', label: 'Settings', hint: 'Connection, capture, about' },
]
    ...
    <Screen scroll>
      ...
      <Card className="mb-4">
        ...
        {state.status === 'paired' && state.mock ? (
          ...
          <Text className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">daemon {state.version}</Text>
        ) : null}
      </Card>
