...
function Meter({ pct, color }: { pct: number; color: string }) {
  return (
    <View className="h-2 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-800">
      <View style={{ width: `${Math.min(Math.max(pct, 0), 100)}%`, backgroundColor: color }} className="h-2 rounded-full" />
    </View>
  )
}
        ...
        <View>
          <Card className="mb-4">
            <View className="flex-row">
              <Stat label="captures" value={q.data.total_captures.toLocaleString()} />
              <Stat label="active vs avg" value={`${q.data.comparison.active.current_pct}%`} hint={`base ${q.data.comparison.active.baseline_pct}%`} />
              <Stat label="distracted" value={`${q.data.comparison.distracted.current_pct}%`} hint={`base ${q.data.comparison.distracted.baseline_pct}%`} />
            </View>
          </Card>
          ...
          {q.data.productivity_states.length > 0 ? (
            <View className="mb-4">
              <SectionTitle>Productivity</SectionTitle>
              <Card>
                {q.data.productivity_states.map((s) => (
          ...
          {q.data.top_apps.length > 0 ? (
            <View className="mb-4">
              <SectionTitle>Top apps</SectionTitle>
              <Card>
                {q.data.top_apps.map((a) => (
                  <View key={a.app_name} className="mb-3">
                    <View className="mb-1 flex-row items-center justify-between">
                      <Text className="text-sm text-slate-700 dark:text-slate-200">{a.app_name}</Text>
          ...
          {q.data.recurring_activities.length > 0 ? (
            <View className="mb-4">
              <SectionTitle>Recurring</SectionTitle>
              <Card>
                {q.data.recurring_activities.map((r, i) => (
                  <View key={i} className={i > 0 ? 'mt-3 border-t border-slate-100 pt-3 dark:border-slate-800' : ''}>
                    <Text className="text-sm text-slate-800 dark:text-slate-100">{r.canonical_summary}</Text>
